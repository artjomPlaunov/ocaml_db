  ...
  object (self)
    ...
    method private execute_select ~output data =
      ...
      | table :: _ ->
          ...
          let filtered_scan =
            match Select_data.predicate data with
    ...
    method private execute_insert data =
      ...
      let tbl_scan =
        Table_scan.make ~tx ~tbl_name:(Insert_data.table_name data) ~layout
      in
      tbl_scan#insert;
      List.iter2
        (fun field value ->
    ...
    method private execute_delete data =
      ...
      let tbl_scan =
        Table_scan.make ~tx ~tbl_name:(Delete_data.table_name data) ~layout
      in
    ...
    method private execute_modify data =
      ...
      let tbl_scan =
        Table_scan.make ~tx ~tbl_name:(Modify_data.table_name data) ~layout
      in
      ...
      while select#next do
        ...
        | Scans__Expression.Const c ->
            select#set_val ~field_name:(Modify_data.field_name data) ~value:c
