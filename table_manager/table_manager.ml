...
let create_table table_mgr tbl_name schema tx =
  ...
  (* insert record into table catalogalog *)
  let tbl_catalog =
    Table_scan.make ~tx ~tbl_name:"tablecatalogalog"
  ...
  (* insert record into field catalogalog for each field *)
  let fld_catalog =
    Table_scan.make ~tx ~tbl_name:"fieldcatalogalog"
...
let make ~is_new ~tx =
  ...
  if is_new then (
    create_table table_mgr "tablecatalogalog" tbl_catalog_schema tx;
    create_table table_mgr "fieldcatalogalog" fld_catalog_schema tx);
  table_mgr
