...
let add_field schema field_name ty length =
  schema.fields <- field_name::(schema.fields);
  Hashtbl.add schema.info field_name {ty; length}

let add_int_field schema field_name =
  add_field schema field_name Integer 0
...
let has_field schema field_name =
  List.mem field_name schema.fields
