...
module To_test = struct
  let cleanup_test_env env = Transaction.commit env.transaction
  ...
  let setup_simple_select_data () =
    ...
    Table_manager.create_table ~table_mgr ~tbl_name:"T1" ~schema
      ~tx:env.transaction;
    let layout =
      Table_manager.get_layout ~table_mgr ~tbl_name:"T1" ~tx:env.transaction
    in
  ...
  let execute_simple_select_query () =
    let env = Test_utils.make_test_env ~db_name:"simple_select_test" in
    let interpreter_env =
      Interpreter.make_env ~tx:env.transaction
        ~buffer_manager:env.buffer_manager
    in
    ...
    let filename =
      Test_utils.write_query_output ~db_name:"simple_select_test"
        ~query_name:"simple_select" ~output:(Buffer.contents output)
    in
  ...
  let execute_insert_and_select_query () =
    ...
    cleanup_test_env env;
    let filepath =
      Test_utils.write_query_output ~db_name:"insert_select_test"
        ~query_name:"insert_and_select" ~output:(Buffer.contents output)
    in
  ...
  [
    Alcotest.test_case "simple select" `Quick (fun () ->
        Alcotest.(check string)
          "simple select results" simple_select_expected
          (To_test.test_simple_select ()));
    Alcotest.test_case "insert and select" `Quick (fun () ->
        Alcotest.(check string)
          "insert and select results" insert_select_expected
          (To_test.test_insert_and_select ()));
  ]
