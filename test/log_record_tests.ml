module To_test = struct
  ...
  let test_create_int_log () =
    ...
    let _ =
      Log_record.write_update_int_log_record env.log_manager 15 block 255
        (Int32.of_int 15)
    in
  ...
  let test_create_multiple_records () =
    ...
    for i = 0 to 2 do
      ...
      let _ =
        Log_record.write_update_int_log_record env.log_manager i block i
          (Int32.of_int (100 + i))
      in
      ()
    done;
    ...
    for i = 0 to 2 do
      let block = Block_id.make ~filename:"fname" ~block_num:i in
      let _ =
        Log_record.write_update_string_log_record env.log_manager i block i
          ("string value" ^ string_of_int i)
      in
      ()
    done;
...
let checkpoint_and_int_expected =
  "<CHECKPOINT>\n<UPDATE INT 15 fname, 1 255 15>\n"

let multiple_records_expected =
  {|<START 6>
<START 6>
<START 6>
<ROLLBACK 4>
...
let test_create_int_log () =
  Alcotest.(check string)
    "create int log record" int_log_expected
    (To_test.test_create_int_log ())

let test_create_checkpoint_and_int () =
  Alcotest.(check string)
    "create checkpoint and int records" checkpoint_and_int_expected
    (To_test.test_create_checkpoint_and_int ())

let test_create_multiple_records () =
  Alcotest.(check string)
    "create multiple record types" multiple_records_expected
    (To_test.test_create_multiple_records ())

let all_tests () =
  [
    Alcotest.test_case "create int log" `Quick test_create_int_log;
    Alcotest.test_case "create checkpoint and int" `Quick
      test_create_checkpoint_and_int;
    Alcotest.test_case "create multiple records" `Quick
