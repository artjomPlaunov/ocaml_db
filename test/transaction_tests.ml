module To_test = struct
  ...
  let test_transaction0 () =
    ...
    Transaction.set_int ~tx:tx ~block ~offset:80 ~value:(Int32.of_int 255) ~to_log:true;
...
let test_transaction0 () =
  Alcotest.(check string)
    "string equality" "<START 1><UPDATE INT 1 testfile, 1 80 255><COMMIT 1>" (To_test.test_transaction0 ())

let test_transaction1 () =
  Alcotest.(check string) "string equality"
    "<START 2><UPDATE INT 2 testfile, 1 80 1><UPDATE STRING 2 testfile, 1 40 one><COMMIT 2><START 3><UPDATE INT 3 testfile, 1 80 2><UPDATE STRING 3 testfile, 1 40 one!><COMMIT 3><START 4><UPDATE INT 4 testfile, 1 80 9999><ROLLBACK 4><START 5>"
