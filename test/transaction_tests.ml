module To_test = struct
  ...
  let test_transaction0 () =
    let env =
      Test_utils.make_transaction_test_env ~db_name:"transaction_tests0"
        ~block_size:1024 ~num_buffers:8
    in
    let tx =
  ...
  let test_transaction1 () =
    let env =
      Test_utils.make_transaction_test_env ~db_name:"transaction_tests"
        ~block_size:1024 ~num_buffers:8
    in
    let tx1 =
      Transaction.make ~file_manager:env.file_manager
    ...
    let ival =
      Transaction.get_int32 ~tx:tx2 ~block ~offset:80 |> Int32.to_int
    in
