module To_test = struct
  ...
  let single_buffer_update () =
    ...
    let after_msg =
      Printf.sprintf "Buffer contents after update: %d\n" (n + 1)
    in
  ...
  let multiple_buffer_update () =
    ...
    let after_msg1 =
      Printf.sprintf "Buffer contents at offset 0 after first update: %d\n"
        (n + 1)
    in
    Buffer_manager__Db_buffer.set_modified buf1 1 0;
    Buffer_manager.unpin env.buffer_manager buf1;
    ...
    let after_msg2 =
      Printf.sprintf "Buffer contents at offset 80 after second update: %d\n"
        9999
    in
...
(* Expected outputs *)
let single_buffer_expected =
  "Buffer contents before update: 0\nBuffer contents after update: 1\n"
...
let double_pin_expected =
  "Attempting to pin block 1...Exception: No available Buffers...Successfully \
   pinned..."

(* Test drivers *)
let test_single_buffer_update () =
  Alcotest.(check string)
    "single buffer update log" single_buffer_expected
    (To_test.single_buffer_update ())

let test_multiple_buffer_update () =
  Alcotest.(check string)
    "multiple buffer update log" multiple_buffer_expected
    (To_test.multiple_buffer_update ())

let test_pin_unpin () =
  Alcotest.(check string)
    "pin unpin message" pin_unpin_expected (To_test.pin_unpin ())

let test_double_pin () =
  Alcotest.(check string)
    "double pin message" double_pin_expected (To_test.double_pin ())
